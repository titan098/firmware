...
RUN apk add --no-cache bash g++ libstdc++-dev linux-headers zip git ca-certificates gpsd-dev libgpiod-dev yaml-cpp-dev bluez-dev \
...
RUN apk add --no-cache libstdc++ gpsd libgpiod yaml-cpp libusb i2c-tools \
