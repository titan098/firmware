...
RUN apk --no-cache add \
        bash g++ libstdc++-dev linux-headers zip git ca-certificates libgpiod-dev gpsd-dev yaml-cpp-dev bluez-dev \
...
RUN apk --no-cache add \
        shadow libstdc++ libgpiod gpsd yaml-cpp libusb i2c-tools libuv \
